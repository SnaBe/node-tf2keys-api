...
**_Important note: TF2Keys.net is still under development and a lot of the APIs functionality is therefore currently unavaible. I've deployted a test server running the API, it does work as intended, but has a limited item list. Hence the module returns an error when requesting some item prices. I've only made the module public for reporting errors, requesting features & testing the servers limits For testign purposes I've disabled the need of an API Key.._**
